...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("polariscoinnetworkMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("polariscoinnetworkMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("polariscoinnetworkMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("polariscoinnetwork-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running polariscoinnetworkMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
